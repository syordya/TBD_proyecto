...
def windowCharacter(x):
    ...
    data_dm = scale(tmp,with_mean=True, with_std=False) # Dregadacion de datos
...
for file in glob.glob('output/*-activity.csv'):
    ...
    for site in placements:
        ...
        for act in activities:
            act_site_fileObj = (site_fileObj.loc[site_fileObj['Actividad'] == act])
            try:
